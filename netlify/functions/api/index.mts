...
import { reservarEnsayo, consumirLlamada, presupuestoLlamadas, estadoCuota, limites } from './lib/cuota.mts';
...
export default async (req: Request, context: Context) => {
  ...
  try {
    ...
    if (ruta === 'sintesis' && req.method === 'POST') {
      ...
      return respuestaLenta(async () => ({ sintesis: await sintesis(idea, resumen, lente, clave) }));
    }
